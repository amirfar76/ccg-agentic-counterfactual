...
int main (int argc, char *argv[])
{
  ...
  // Install Mobility: eNB fixed, UEs on a circle
  ...
  // Route to the internet (use the helper instance, not a static call)
  Ptr<Ipv4StaticRouting> rhRouting =
      ipv4RoutingHelper.GetStaticRouting (remoteHost->GetObject<Ipv4> ());
  rhRouting->AddNetworkRouteTo (Ipv4Address ("7.0.0.0"),
                                Ipv4Mask ("255.0.0.0"),
                                1);
